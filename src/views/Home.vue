<template>
  <main class="app-home px-4 mt-8">
    <v-row>
      <v-col cols="12" sm="4" md="3" v-if="drawer">
        <v-card
          flat
          dark
          :class="[
        ...
        >
          <span
            style="position: absolute; right: -0.75rem; top: -0.75rem; z-index: 1;"
          >
            <v-btn
              fab
              x-small
          ...
          <v-row no-gutters class="pa-3">
            ...
            >
              ...
              >
                <v-icon left v-if="isSelected(language.code)">
                  mdi-check
                </v-icon>
                <v-icon left v-else>
      ...
      <v-col>
        <div class="d-flex align-center mb-5">
          ...
          <span v-if="$vuetify.breakpoint.mdAndUp" :class="{ 'ml-5': !drawer }">
    ...
    >
      ...
      <template v-slot:action="{ attrs }">
        <v-btn small v-bind="attrs" @click="snackbar = false" icon>
...
export default {
  ...
  computed: {
    ...
    snackbar: {
      ...
      set(val) {
        this.$store.commit("toggleSnackbar", val);
      },
    },
    message() {
  ...
  methods: {
    getStoredLanguages() {
      ...
      else this.$store.dispatch("getLanguages");
    },
    isSelected(code) {
    ...
    async toggleLanguage(language) {
      ...
      let index = this.translations.findIndex(
        (el) => language.code === el.target
      );
      if (index >= 0) {
